...
def rescale_img(img_object, width, height, scale=False):
  ...
  if scale:
    ...
    x = int(width / ratio)
    img_object = cv2.resize(img_object, (x, height))
  else:
...
def cut_boxes(path_to_save, image_path, boxes, scores, threshold=0.5):
  image = cv2.imread(image_path)
  # visualize detections
  i = 1
  for box, score, label in zip(boxes[0].astype(int), scores[0]):
      # scores are sorted so we can break
      if score < threshold:
        continue
      save_box(path_to_save + str(i) + ".png", image, box)
      i += 1

def save_box(path_to_save, image, box):
